...
Flag.Event = function(ID,Map,Event,HeroLocation,Floor,callback){
  ...
  switch(ID){
    case 0:
      if(Flag.Switch[0]){
        Message = ["[Npc=3,仙子]别再看剧情了，有这个时间别人都AK了"];
      ...
      Message = ["[Hero]这里是哪里？怎么二二零三年还在搞像素世界啊？","[Npc=3,仙子]你说得对，但是《魔塔传奇》是由赛博安全协会自主克隆的经典封闭世界冒险游戏","[Npc=3,仙子]游戏发生在一个被称作{没想好}的幻想世界","[Npc=3,仙子]在这里，被选中的人将被授予{攻防属性}，引导道具之力。","[Npc=3,仙子]你将扮演一位名为{匿名}的勇敢角色，在固定的行程中找到效果各异，能力独特的装备","[Npc=3,仙子]用他们击败强敌，到达八层魔塔的顶端——同时，逐步发掘{公主}的真相"];
      Event.ShowMessageList(Message,function(){
        Flag.Move = false;
        Event.UnlockMove();
        Event.Enable("Controller");
